...
| Mixed-Q5 exact-131K owner | Reduce persistent full-attention KV and prompt-snapshot residency before another maximum-capacity run | A130 removes dense attention scratch, leaving sixteen BF16 K/V pairs at **8 GiB** by 131,072 tokens plus functional-update/snapshot lifetimes. Snapshot elision and an exact native FP8 cache are the active candidates; the 32K result is not an exact-131K qualification |
