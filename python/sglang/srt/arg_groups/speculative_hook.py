...
def _validate_topk1_delta_proposal(server_args: ServerArgs, enabled: bool) -> None:
    ...
    speculative_algorithm = (view.speculative_algorithm or "").upper()
    if (
        sys.platform != "win32"
        or view.device != "cuda"
        or speculative_algorithm not in ("NEXTN", "EAGLE", "EAGLE3")
        or not view.speculative_use_rejection_sampling
...
def _handle_eagle_family(server_args: ServerArgs) -> None:
    ...
    if cfg.speculative_eagle_topk > 1:
        ...
        verify_backend = (
            decode_backend
            if cfg.speculative_attention_mode == "decode"
    ...
    if cfg.speculative_tree_sampling_mode == "swor":
        if cfg.speculative_algorithm not in ("EAGLE", "EAGLE3"):
            raise NotImplementedError(
    ...
    if (
        cfg.speculative_swor_collect_path_stats
        and cfg.speculative_tree_sampling_mode != "swor"
    ):
        raise ValueError(
    ...
    if (
        cfg.speculative_swor_collect_overlap_stats
        and cfg.speculative_tree_sampling_mode != "swor"
    ):
        raise ValueError(
    ...
    if (
        cfg.speculative_swor_topology is not None
        and cfg.speculative_tree_sampling_mode != "swor"
    ):
        raise ValueError(
    ...
    if server_args.speculative_pq_capture_path is not None:
        ...
        if cfg.cuda_graph_max_bs_decode != 1:
            raise ValueError(
        ...
        if (
            not cfg.speculative_use_rejection_sampling
    ...
    if cfg.speculative_draft_sampling_top_k is not None:
        if cfg.speculative_draft_sampling_top_k < 1:
            raise ValueError(
                "--speculative-draft-sampling-top-k must be >= 1, got "
                f"{cfg.speculative_draft_sampling_top_k}."
            )
        if cfg.speculative_use_rejection_sampling:
            logger.info(
                "Sparse aligned draft proposal sampling is enabled "
                "(speculative_draft_sampling_top_k=%d).",
                cfg.speculative_draft_sampling_top_k,
            )
        else:
            logger.info(
    ...
    tree_depth_discount = cfg.speculative_tree_depth_discount
