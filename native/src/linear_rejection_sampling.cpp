...
shape_error(LinearRejectionSamplingArgument argument, uint64_t actual,
...
std::string_view linear_rejection_sampling_argument_name(
  ...
  case LinearRejectionSamplingArgument::kCorrectedLogits:
    return "corrected_logits";
  case LinearRejectionSamplingArgument::kLogNormalizers:
    return "log_normalizers";
  case LinearRejectionSamplingArgument::kTemperatures:
    return "temperatures";
  default:
...
NativeRuntimeError validate_linear_rejection_sampling_shape(
  ...
  if (shape.num_slots < 2) {
    return shape_error(LinearRejectionSamplingArgument::kNumSlots,
                       shape.num_slots, 2);
  }
  if (shape.num_slots > kLinearRejectionSamplingMaxNumSlots) {
    return shape_error(LinearRejectionSamplingArgument::kNumSlots,
                       shape.num_slots, kLinearRejectionSamplingMaxNumSlots);
  }
  if (shape.vocab_size == 0) {
    return shape_error(LinearRejectionSamplingArgument::kVocabSize,
                       shape.vocab_size, 1);
  }
  if (shape.vocab_size >
  ...
  if (shape.num_out_tokens != shape.num_slots) {
    return shape_error(LinearRejectionSamplingArgument::kOutTokens,
