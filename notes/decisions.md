...
| Linear-attention Q5 `qkv`/`z` launch sharing | Reject load-time row concatenation and runtime split; admit only a direct two-output kernel | The isolated pair improves **0.426645998 -> 0.420592346 ms**, but full-model candidates **19.441971742 / 19.406059113** lose to adjacent disabled control **19.469521945 tok/s**. A materially different native kernel must consume the original tensors and emit both outputs directly; see PERF-FA136 |
