...
void validate(const TtftOptions &o) {
  if (o.base_url.empty() || o.model.empty() || o.label.empty() ||
      o.input_tokens < 1 || o.input_tokens > 199000 || o.output_tokens < 1 ||
      o.output_tokens > 200000 - o.input_tokens || o.samples < 1 ||
      o.samples > 100 || !std::isfinite(o.timeout_seconds) ||
      o.timeout_seconds <= 0 || o.timeout_seconds > 3600) {
...
JsonValue get_json(HttpTransport &transport, const std::string &url,
  ...
  if (!response.ok() || response.response.status_code != 200) {
    throw std::runtime_error("GET " + url +
                             " failed: " + response.error.message + " HTTP " +
...
std::vector<std::int64_t> tokenize(HttpTransport &transport,
  ...
  const auto response =
      request_json(transport, base_url(o.base_url) + "/v1/tokenize",
                   Object{{"model", o.model},
                          {"messages", messages_for(prompt.content)},
...
void check_server(const JsonValue &server, const TtftOptions &o) {
  ...
  if (!server.at("enable_cache_report").as_bool()) {
    throw std::runtime_error(
        "TTFT prefix probe requires --enable-cache-report");
  }
  if (server.at("disable_radix_cache").as_bool()) {
    throw std::runtime_error(
        "prefix reuse cannot be measured with radix cache disabled");
  }
  if (server.at("served_model_name").as_string() != o.model ||
      server.at("max_running_requests").as_int() != 1) {
    throw std::runtime_error(
        "expected selected model and one-request scheduler");
  }
  const auto needed =
      std::max(o.input_tokens, o.continuation_tokens) + o.output_tokens;
  if (server.at("context_length").as_int() < needed ||
      server.at("max_total_tokens").as_int() < needed) {
    throw std::runtime_error(
        "request does not fit the resolved context/token pool");
  }
}
...
JsonValue model_identity(const JsonValue &model, const TtftOptions &o) {
  if (model.at("served_model_name").as_string() != o.model ||
    ...
    throw std::runtime_error(
        "expected language-only Qwen reasoning/tool model");
  }
  Object result;
  for (const auto key :
  ...
  for (const auto key : {"model_path", "tokenizer_path"}) {
    ...
    if (value.empty())
      throw std::runtime_error("empty live model/tokenizer path");
...
TtftOptions parse_ttft_arguments(std::span<const std::string_view> args) {
  ...
  for (std::size_t i = 0; i < args.size(); ++i) {
    ...
    if (!seen.insert(key).second || i + 1 == args.size() ||
      ...
      throw std::invalid_argument("duplicate option or missing value: " +
                                  std::string(key));
    }
    ...
    if (key == "--base-url")
      ...
      options.input_tokens = integer();
    else if (key == "--output-tokens")
      options.output_tokens = integer();
    else if (key == "--samples")
      options.samples = integer();
    else if (key == "--continuation-tokens")
      options.continuation_tokens = integer();
    else if (key == "--timeout") {
      const auto parsed = std::from_chars(
          value.data(), value.data() + value.size(), options.timeout_seconds);
      if (parsed.ec != std::errc{} ||
          parsed.ptr != value.data() + value.size()) {
...
void run_ttft_benchmark(HttpTransport &transport, const TtftOptions &o,
  ...
  validate(o);
  if (!emit || !now)
    throw std::invalid_argument("TTFT callbacks must be set");
  const auto base = base_url(o.base_url);
  const auto server =
      get_json(transport, base + "/server_info", o.timeout_seconds);
  ...
  emit(Object{
      {"kind", "manifest"},
      {"protocol", "native-ttft-prefix-v1"},
      ...
      {"continuation_prompt_sha256",
       continuation ? JsonValue(sha256_hex(continuation->content))
                    : JsonValue(nullptr)},
      {"sampling", sampled_controls()},
      {"server_profile", server_profile(server)},
      {"model_identity", identity},
      {"startup_identity_sha256", sha256_hex(server.at("startup_time").dump())},
      {"ttft_boundary",
       "dispatch_to_post_json_first_nonempty_reasoning_or_content"},
      {"warmup_sequences", 1},
  });
  ...
  const auto measure = [&](const CalibratedPrompt &prompt, std::string phase,
    ...
    const auto &reported =
        result.at("diagnostics").at("cached_prompt_tokens_reported");
    ...
    if (phase == "cache_flushed" && cached != 0) {
      throw std::runtime_error(
          "cache-flushed request reports prefix hits; server not isolated");
    }
    ...
    emit(result);
    if (sample != 0)
      timings[phase].push_back(result.at("ttft_s").as_double());
  };

  // Full-shape warmup includes retained-prefix paths; its receipts are
  // excluded. Each measured window then has exactly the same bounded cache
  // sequence.
  ...
  if (server_profile(final_server) != server_profile(server) ||
      ...
      model_identity(
          ...
          identity) {
    throw std::runtime_error(
        "server changed during TTFT probe; no summary emitted");
  }
  for (const auto &[phase, values] : timings) {
    ...
    emit(Object{
        ...
        {"phase", phase},
        {"samples", values.size()},
        {"ttft_s_samples", std::move(samples)},
