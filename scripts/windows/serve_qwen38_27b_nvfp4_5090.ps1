...
Requests that omit sampling values use the checkpoint's generation
configuration, which the launcher checks for the qualified temperature of 1.0.
The checkpoint is served from its own path, so the FlashInfer tuning cache key
and the selected FP4 tactics stay stable across launches.
#>
...
# The served path is part of the FlashInfer tuning cache key. Serving the
# checkpoint from its own path keeps that key, and so the selected FP4
# tactics, stable across launches. Sampling defaults come from the
# checkpoint's generation configuration, which must carry the qualified
# temperature.
$GenerationConfigPath = Join-Path $ResolvedModelPath 'generation_config.json'
$GenerationConfig = ConvertFrom-Json -AsHashtable -InputObject (
    Get-Content -LiteralPath $GenerationConfigPath -Raw
)
if ($GenerationConfig -isnot [System.Collections.IDictionary]) {
    throw "generation_config.json must contain a JSON object: $GenerationConfigPath"
}
if (-not $GenerationConfig.ContainsKey('temperature') -or
    [double] $GenerationConfig['temperature'] -ne $SamplingTemperature) {
    throw ('generation_config.json temperature must be {0}: {1}' -f $SamplingTemperature, $GenerationConfigPath)
}
