...
| Mixed-Q5 exact-131K owner | PERF-A134 affine-Q8/G64 K/V with append-only snapshots, fixed-memory prefill, and split-history decode | A133's metadata-only snapshot passes exact rollback/overwrite invariants, but a final 131,072-slot BF16 reserve fails exact 32K after about 16 seconds: scheduler RSS is **18,225,056 KiB** before the request and final K/V adds 8 GiB against the approximately 25 GiB Metal working set. Q8/G64 reduces final K/V residency to approximately **4.25 GiB**; the 32K A130 result is not an exact-131K qualification |
