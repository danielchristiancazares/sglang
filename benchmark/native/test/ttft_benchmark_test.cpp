...
HttpTimePoint at(std::chrono::milliseconds value) { return HttpTimePoint{} + value; }

class Protocol final : public HttpTransport {
public:
  Json server = Object{
      {"enable_cache_report", true}, {"disable_radix_cache", false},
      {"served_model_name", "qwen3.8-27b"}, {"max_running_requests", 1},
      {"context_length", 200000}, {"max_total_tokens", 200000},
      {"chunked_prefill_size", 4096}, {"max_mamba_cache_size", 5},
      {"startup_time", Object{{"unix", 123456}}}};
  Json model = Object{
      {"served_model_name", "qwen3.8-27b"}, {"is_generation", true},
      {"model_path", "private/model"}, {"tokenizer_path", "private/tokenizer"},
      {"weight_version", nullptr}, {"load_format", "auto"},
      {"reasoning_parser", "qwen3"}, {"tool_call_parser", "qwen3_coder"},
      {"has_image_understanding", false}, {"has_audio_understanding", false},
      {"model_type", "qwen"}, {"architectures", Array{"Qwen"}},
  ...
  HttpResult perform(const HttpRequest &request,
    ...
    if (request.url.ends_with("/server_info")) {
      ...
      if (++server_reads > 1) {
        if (restart) returned.as_object()["startup_time"] = Object{{"unix", 999999}};
        if (change_config) returned.as_object()["chunked_prefill_size"] = 7680;
      }
    ...
    if (request.url.ends_with("/model_info")) {
      ...
      auto returned = model;
      if (++model_reads > 1 && change_model) returned.as_object()["weight_version"] = "changed";
      response.body = returned.dump();
      return result;
    }
    if (request.url.ends_with("/v1/tokenize")) {
      ...
      if (bad_token_ids && !ids.empty()) ids.back() = -1;
      response.body = Json(Object{{"count", ids.size()}, {"tokens", ids}}).dump();
      return result;
    }
    ...
    const auto count = tokens(payload.at("messages").at(0).at("content").as_string()).size();
    const auto completion = payload.at("max_completion_tokens").as_int();
    Object usage{{"prompt_tokens", count + (wrong_counts ? 1 : 0)},
                 {"completion_tokens", completion}, {"total_tokens", count + completion}};
    if (malformed_cached) {
      usage["prompt_tokens_details"] = Object{{"cached_tokens", *malformed_cached}};
    ...
    const auto send = [&](const std::string &bytes, std::chrono::milliseconds received) {
      response.body += bytes;
      response.body_bytes += bytes.size();
      if (!callback(bytes, at(received))) response.stopped_early = true;
    };
    ...
    send("data: " + Json(Object{
        {"choices", Array{Object{{"delta", Object{}},
                                  {"finish_reason", wrong_finish ? "stop" : "length"}}}},
        {"usage", usage}}).dump() + "\n", 7ms);
    if (!truncate_done) send("data: [DONE]\n", 10ms);
    return result;
  }
...
private:
  ...
  Array tokens(std::string_view content) const {
    ...
    while (content.substr(position).starts_with(kFillerUnit)) {
      ids.emplace_back(5);
      if (unreachable_count) ids.emplace_back(6);
      position += kFillerUnit.size();
    }
    if (position != content.size()) throw std::runtime_error("unexpected prompt shape");
    ids.emplace_back(9);
    return ids;
  }
};
...
void run(Protocol &transport, const TtftOptions &o, std::vector<Json> &records) {
  run_ttft_benchmark(transport, o, [&](const Json &value) { records.push_back(value); },
                     [] { return at(5ms); });
}
...
void SequenceWarmupAndControls() {
  ...
  for (const auto &action : transport.operations) {
    if (action == "flush" || action == "stream") actions.push_back(action);
  }
  ...
  for (const auto &payload : transport.payloads) {
    ...
    REQUIRE(payload.at("chat_template_kwargs").at("preserve_thinking").as_bool());
  ...
  REQUIRE(records[1].at("diagnostics").at("cached_prompt_tokens_reported").is_null());
...
void NoContinuationAndZeroReporting() {
  for (const bool explicit_zero : {false, true}) {
    ...
    for (std::size_t i = 1; i <= 4; ++i) {
      ...
      REQUIRE(records[i].at("diagnostics").at("cached_prompt_tokens_reported").is_null() ==
              !explicit_zero);
    }
  }
}
...
void InvalidServerStopsBeforeCalibrationOrFlush() {
  ...
  std::vector<Json> records;
  REQUIRE(native_test::throws([&] { run(transport, options(), records); }));
  REQUIRE(transport.operations == std::vector<std::string>({"server", "model"}));
}

void FailedOrIncompleteStreamsNeverFlushAgainOrSummarize() {
  for (int failure = 0; failure < 7; ++failure) {
    Protocol transport;
    if (failure == 0) transport.fail_stream = 2;
    if (failure == 1) transport.truncate_done = true;
    if (failure == 2) transport.no_output = true;
    if (failure == 3) transport.wrong_counts = true;
    if (failure == 4) transport.wrong_finish = true;
    if (failure == 5) transport.flush_hit = true;
    if (failure == 6) transport.malformed_cached = 11;
    std::vector<Json> records;
    REQUIRE(native_test::throws([&] { run(transport, options(), records); }));
    REQUIRE(transport.operations.back() == "stream");
    REQUIRE(std::count(transport.operations.begin(), transport.operations.end(), "flush") == 1);
    REQUIRE(std::none_of(records.begin(), records.end(), [](const Json &record) {
...
void InvalidCalibrationCannotStartInference() {
  for (const bool invalid_ids : {true, false}) {
    ...
    auto o = options();
    if (!invalid_ids) { o.input_tokens = 3; o.continuation_tokens = 0; }
    std::vector<Json> records;
    REQUIRE(native_test::throws([&] { run(transport, o, records); }));
    REQUIRE(records.empty());
    REQUIRE(transport.stream_count == 0);
    REQUIRE(std::count(transport.operations.begin(), transport.operations.end(), "flush") == 0);
  }
}

void OptionsFailBeforeNetwork() {
  const std::vector<std::vector<std::string_view>> invalid{
      {"--samples", "0"}, {"--samples", "101"}, {"--samples", "2", "--samples", "2"},
      {"--timeout", "nan"}, {"--timeout", "0"}, {"--timeout", "3601"},
      {"--input-tokens", "199001"}, {"--input-tokens", "199000", "--output-tokens", "1001"},
      {"--continuation-tokens", "6213"}, {"--continuation-tokens", "-1"},
      {"--base-url", "http://localhost:30000/v1"}, {"--unknown", "1"}, {"--model"}};
  for (const auto &args : invalid) {
    REQUIRE(native_test::throws([&] { static_cast<void>(parse_ttft_arguments(args)); }));
  }
...
void ReceiptWriteFailureStopsFurtherWork() {
  ...
  REQUIRE(native_test::throws([&] {
    run_ttft_benchmark(transport, options(), [&](const Json &) {
      if (++receipts == 2) throw std::runtime_error("output failed");
    }, [] { return at(5ms); });
  }));
...
int main() {
  const std::array tests{
      std::pair{"SequenceWarmupAndControls", SequenceWarmupAndControls},
      std::pair{"NoContinuationAndZeroReporting", NoContinuationAndZeroReporting},
      std::pair{"InvalidServerStopsBeforeCalibrationOrFlush", InvalidServerStopsBeforeCalibrationOrFlush},
      std::pair{"FailedOrIncompleteStreamsNeverFlushAgainOrSummarize", FailedOrIncompleteStreamsNeverFlushAgainOrSummarize},
      std::pair{"ConfigurationRestartAndLiveModelChangeWithholdSummaries", ConfigurationRestartAndLiveModelChangeWithholdSummaries},
      std::pair{"InvalidCalibrationCannotStartInference", InvalidCalibrationCannotStartInference},
      std::pair{"OptionsFailBeforeNetwork", OptionsFailBeforeNetwork},
      std::pair{"ReceiptWriteFailureStopsFurtherWork", ReceiptWriteFailureStopsFurtherWork}};
