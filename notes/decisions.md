...
| Stock MPS FP8 KV cache | Closed on PyTorch 2.10.0 MPS dtype support | A direct `torch.float8_e4m3fn` allocation/conversion probe raises the framework's unsupported-MPS-dtype `TypeError`; the generic SGLang pool cannot allocate. Reopen with MPS float8 tensor support or a native uint8-backed quantize/dequantize cache owner. |
