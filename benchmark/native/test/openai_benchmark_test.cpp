...
[[nodiscard]] bool DiagnosticsAreAdditiveAndDoNotRedefineTtft() {
  ...
  transport.chunks = {
      ...
      {"data: {\"choices\":[{\"delta\":{\"reasoning_content\":\"x\"}}]}", at(700ms)},
      {"\n", at(1s)},
      {"data: {\"choices\":[{\"delta\":{},\"finish_reason\":\"length\"}],"
...
[[nodiscard]] bool CachedUsagePreservesUnknownZeroAndRejectsMalformed() {
  for (const auto &details : std::vector<JsonValue>{
           nullptr, JsonValue::object{}, JsonValue::object{{"cached_tokens", nullptr}},
           JsonValue::object{{"cached_tokens", 0}}, JsonValue::object{{"cached_tokens", 10}}}) {
    StreamAccumulator accumulator;
  ...
  for (const auto &details : std::vector<JsonValue>{
           JsonValue::array{}, "invalid", JsonValue::object{{"cached_tokens", -1}},
           JsonValue::object{{"cached_tokens", 11}}, JsonValue::object{{"cached_tokens", 2.5}},
           JsonValue::object{{"cached_tokens", "2"}}, JsonValue::object{{"cached_tokens", true}}}) {
    StreamAccumulator accumulator;
    CHECK(!accumulator.consume_data_at(
        JsonValue(JsonValue::object{{"usage", JsonValue::object{
            {"prompt_tokens", 10}, {"prompt_tokens_details", details}}}}).dump(), at(1s)));
    // Legacy finalization never interprets the optional cache-details field.
    CHECK(accumulator.finalize(at(0ms), at(2s)).at("prompt_tokens").as_int() == 10);
    CHECK(throws([&] { static_cast<void>(accumulator.cached_prompt_tokens()); }));
  }
  return true;
}
