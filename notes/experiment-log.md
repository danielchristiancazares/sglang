...
### 2026-08-17 00:09 PDT — Mac Pro real-world sampled TPS measured; aggregate 60 TPS distinction restored

- Began on the Intel Mac Pro (`x86_64`, macOS 26.6) at clean `main` HEAD `b45fdef7dd476bbc483aeb63f90b5ba73f84c5ec`, synchronized with `origin/main`. The runtime was `.venv-mac-metal` Python 3.11.15, PyTorch 2.2.2 with MPS available, and the AMD Radeon Pro W6900X 32 GB. No serving process or port-30001 listener existed before the run.
- Used the immutable `/Users/daniel/models/Qwen3.8-27B-Q4_0/Qwen3.8-27B-Q4_0.gguf` checkpoint and `/Users/daniel/models/Qwen3.8-27B-tokenizer`. The successful launch command was `.venv-mac-metal/bin/python -m sglang.launch_server --model-path /Users/daniel/models/Qwen3.8-27B-Q4_0/Qwen3.8-27B-Q4_0.gguf --tokenizer-path /Users/daniel/models/Qwen3.8-27B-tokenizer --served-model-name qwen3.8-27b --load-format gguf --device mps --dtype float32 --attention-backend torch_native --sampling-backend pytorch --language-model-only --reasoning-parser qwen3 --disable-radix-cache --max-running-requests 1 --max-total-tokens 7680 --max-mamba-cache-size 4 --host 127.0.0.1 --port 30001`.
- Two preceding launches requested pools of 8192 and 7936. Both reached the native Metal decode path and failed during the internal startup generation with `RuntimeError: native Metal decode attention supports at most 7936 cache slots`. The C++ guard checks physical `key_cache.size(0) <= 7936`; the 7936 logical launcher value still produced a physical cache shape above that ceiling. Both failed trees cascaded out and port 30001 cleared before replacement. The 7680 pool left sufficient room for exact `6213+512`, loaded 15.85 GB of weights, allocated 0.70 GB of Mamba state and about 0.94 GB of FP32 KV, reported 15.08 GB available, and passed the internal generation. The live tree was rooted at PID 26994 with scheduler PID 27023 and detokenizer PID 27024.
- Real-world sampled command: `.venv-mac-metal/bin/python scripts/windows/bench_openai_stream.py --base-url http://127.0.0.1:30001 --input-tokens 6213 --output-tokens 512 --warmup-output-tokens 512 --temperature 1.0 --top-p 0.95 --top-k 20 --presence-penalty 1.5`. It performed an exact-shape warmup, flushed the cache, and then measured an ordinary unseeded request with thinking enabled and preserved.
- The completed retained sample was exact `6213/512`, `finish_reason=length`, **1.788 decode tok/s**, **1.323 end-to-end output tok/s**, `101.229673 s` TTFT, and `387.008627 s` end-to-end. Observed prompt throughput was `61.375 tok/s`; output contained 2,453 reasoning characters and 25 content characters with SHA-256 `221b5ffd0c43c7aafbea95df35addc3dc986489866d33e28a60fcc9d2ea54e09`.
- This is the applicable interactive production result. The retained **62.034 TPS** Mac result in `PERFORMANCE_LOG.md` is aggregate server throughput for 24 simultaneous short-prompt requests, 3,072 generated tokens total, and exact batch-24 Q5_K/Q6_K kernels. It does not describe the token rate observed by one long-context interactive request.
- A second cache-flushed sample produced no result: its client exited with status 137 and the server was absent afterward. It is excluded from the throughput window; the cause was not established before the user paused the run. Cleanup inspection found no SGLang or benchmark process, no port-30001 listener, 96% system memory free, no swap activity, and the W6900X back at zero reported GPU activity with 57 C and 16 W in the post-run snapshot.
- Handoff: the honest result is one completed sample rather than the five-sample comparison window required for a stable benchmark. Resume only on request. Restart with the 7680-slot launch above and collect four or more fresh cache-flushed samples for a stable per-request mean; use the separate batch-24 workload only when aggregate saturated-server capacity is the intended metric.
### 2026-09-08 19:17 PDT — stash conflicts resolved across Metal decode and benchmark ledger

- Began on `main` at `5a816f3d1fd6c0b43c5f7b381641d06384de5fd0`, synchronized with `origin/main`, during a stash application with three unmerged paths: `notes/experiment-log.md`, `python/sglang/kernels/aot/csrc/metal/gguf_q4_0.mm`, and `scripts/windows/bench_openai_stream.py`. Existing staged deletions, the modified Mac attention test, and all untracked benchmark/client files were treated as user-owned and left unchanged.
- Resolved the ledger additively, retaining the complete updated-upstream history followed by the stashed Mac Pro sampled-throughput entry. Resolved the streaming benchmark by retaining upstream's exact prompt/completion/finish-reason validation together with the stashed benchmark behavior.
- Resolved the Metal conflict by preserving both independently added long-context paths: upstream's BF16 `decode_gqa_bf16_online_256` pipeline and the stashed bounded-scratch FP32 `decode_gqa_online_f32` pipeline. Dispatch now selects BF16 by cache dtype, FP32 online attention above 7,936 cache slots, and the original FP32 kernel otherwise. BF16 keeps its 131,073-slot, head-dimension, topology, and Apple-family guards; FP32 keeps the stashed 262,144-slot ceiling. The merged online FP32 kernel also bounds sequence traversal by both cache slots and request stride and validates the request row before indexing.
- Conflict-marker search, Python compilation for all touched scripts and tests, benchmark `--help`, the focused OpenCode benchmark suite (**2 passed**), and `git diff --check` passed. Native MPS execution could not run: `.venv-mac-metal` exists but has no executable `bin/python`, system Python has no Torch installation, and the installed Command Line Tools do not expose `xcrun metal`. No dependency installation, server launch, GPU workload, or process cleanup was performed.
