...
int main(int argc, char** argv) {
  try {
    ...
    std::vector<std::int32_t> arm_ids[2];
    std::uint64_t arm_target[2]{}, arm_mtp[2]{};
    ...
    for (int sample = 0; sample < samples; ++sample) {
      ...
      if (engine.last_prefill_cached_tokens() != 0)
        throw std::runtime_error("cold-prefill benchmark unexpectedly reused prompt state");
      std::cout << "cold_sample=" << sample << " candidate=" << candidate
                << " prompt_tokens=" << prompt.size() << " prefill_seconds=" << prefill_seconds
                << " first_token=" << token << " peak_bytes=" << mlx::core::get_peak_memory() << std::endl;
...
const int arm = candidate ? 1 : 0;
if (sample < 2) {
  arm_ids[arm] = ids;
  arm_target[arm] = target_state;
  arm_mtp[arm] = mtp_state;
} else if (arm_ids[arm] != ids || arm_target[arm] != target_state || arm_mtp[arm] != mtp_state) {
  throw std::runtime_error("repeated cold-prefill arm changed sampled IDs or state");
}
      std::cout << "sample=" << sample << " candidate=" << candidate
                ...
                << " target_equal=" << (reference_target == target_state)
                << " mtp_equal=" << (reference_mtp == mtp_state)
