...
| Q5 bandwidth arm | Pinned mixed 4.951-bpw target at revision `596b8067...8340` | Exact required-language inventory passes; selected standard-MTP A100+A111+A114+A113+A117+A128+A137+A149+A150+A163 reaches two independent means **20.109963672 / 20.107849480 tok/s** with every sample above 20; long-history, exact 131K, and `xhigh` remain open |
...
| Mixed-Q5 actual-work owner | Repair standard-MTP committed history before ranking another checkpoint or verifier kernel | A163 target-only reproduces **19.057906040 tok/s**, while official and optimized MTP checkpoints collapse to **7.839819833 / 7.516953814 tok/s** and widths **1.032258065 / 1.003921569** after 6,237 tokens. Native code clears MTP KV every cycle; official MTPLX retains shifted prompt and accepted-prefix history. |
...
| MTP history staging | PERF-A164 prompt-seeded plus accepted-prefix committed history behind an opt-in | Official-source trace requires prompt hidden rows `0..N-2` paired with tokens `1..N-1`, then restore-and-append of only committed input/hidden pairs per cycle. Stream it in bounded chunks; retain legacy and non-standard draft paths unchanged. |
