...
| Q5 bandwidth arm | Pinned mixed 4.951-bpw target at revision `596b8067...8340` | Exact required-language inventory passes; A163 clears short decode and A164 committed history reaches a clean actual-work mean **23.821754359 tok/s** at width **1.961832061**. Exact served 131K and `xhigh` remain open |
...
| Fused-Q4 parameter layout | PERF-A128 aligned gate/up four-row raw bundles | Two `uint4` parameter loads preserve every BF16 bit and improve two direct windows **19.446907510 -> 19.625370975 tok/s**, **+0.917696%**; the 680 MiB duplicate remains opt-in pending the full capacity contract; signed `f2fcce0c73` |
| Fused-Q4 sigmoid evaluation | PERF-A137 fast exp plus the sole exhaustive BF16 correction | Fast and precise differ only for input `0xc0db`; returning precise sigmoid bits `0x3a8b` there gives zero mismatches over all 65,536 BF16 patterns. Two direct windows improve **19.640965505 -> 19.667930618 tok/s**, **+0.137290%**; signed `24d745ff38`. A precise fallback inside the shader is slower and closed by PERF-FA149 |
| Mixed-Q5 actual-work owner | Retain signed A164 prompt-shifted and accepted-prefix MTP history with the optimized Q4 MTP | Five consecutive long-history samples average **23.821754359 tok/s**, width **1.961832061**, versus the broken empty-cache **7.516953814 tok/s**. Every sample preserves the exact digest, refill count, and requested length. One accepted host-contended observation reached **19.724760551** without acceptance/output drift. |
...
| Affine-Q5 preassembled windows | Reject four-row A139 and row-local A140; retain A100 continuous bits | Both preserve exact output, but the dominant production micro regresses **0.435539514 -> 0.441254344 ms** for A139 and **0.418530771 -> 0.453039271 ms** for A140. Reopen only with a representation that changes extraction cost; see PERF-FA150/151 |
| Affine-Q5 byte-neutral representations | Reject A141 nibble/high-plane, A142/A143 block interleaves, and A144 combined weight/parameter stream | All are bit-exact but regress the dominant projection by **5.800641% / 1.508948% / 0.515803% / 3.662772%**. A143's lone fast first arm does not repeat across six samples. Preserve A100 and require actual byte or arithmetic reduction; see PERF-FA152--155 |
...
| Standard-MTP batch-two verifier | Retain PERF-A149 shared-load Q5, PERF-A150 shared-load fused Q4, and PERF-A163 scalar-order shared-load Q4 head as opt-in kernels | A163's exact head improves **4.195437500 -> 3.440243000 ms**. Independent five-sample model means are **20.109963672 / 20.107849480 tok/s**, every sample above 20 with identical output and acceptance. Signed `6b6d0d15ea` and `94ca4ff7fa`; exact 131K and real-client gates remain open. |
| MTP history staging | Retain PERF-A164 behind `SGLANG_MLX_NATIVE_MTP_COMMITTED_HISTORY`; signed `b92c21d69d` | Prompt hidden rows `0..N-2` pair with tokens `1..N-1`; decode restores speculative state and appends only committed input/hidden pairs. Bounded prompt chunks, target-only fallback advancement, exact one-token-behind invariants, and fail-closed native-sampling checks pass. |
...
| Mixed-Q5 token embedding | On-demand affine row dequantization with signed and unsigned integer indices | Signed `2a65daf1cc` removes **1.702 GiB** of eager BF16 duplicate storage. Signed `e7643c904d` preserves the native sampler's MLX `uint32` feedback alongside prompt `int32`; both focused gathers are bit-exact and floating indices fail closed |
| Mixed-Q5 long-prefill attention | Stock MLX SDPA through 8,192 active tokens, then opt-in fixed-memory Q8/C64 native Metal GQA | Always-on use changes the deterministic sampled trajectory despite tight tensor parity. The selected threshold restores canonical **19.744722973 tok/s** short work and completes independent clean exact `32768+16` at **87.807667 prompt tok/s / 373.179259 s** where stock SDPA OOMs; signed `ee0bf40711` |
| Mixed-Q5 exact-131K owner | Preserve A134 as a capacity primitive; redesign compressed K/V execution before promotion | A133's metadata-only snapshot passes exact rollback/overwrite invariants, but final BF16 K/V cannot fit. A134's approximately **4.25 GiB** Q8/G64 final allocation fits directly, yet reaches only **3.515167775 tok/s** at 8K history versus matched BF16 **7.529148099** at 16K reserve; a 1--32 split sweep does not repair it. Neither the A130 32K pass nor A134 allocation is exact served-131K qualification |
