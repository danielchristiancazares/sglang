...
| Q5 bandwidth arm | Pinned mixed 4.951-bpw target at revision `596b8067...8340` | Exact required-language inventory passes; selected A100+A111+A114 plus A113 token-eval cleanup reaches **19.289499778 tok/s** across two clean paired windows, **+6.444%** over generic mixed execution |
...
| Affine-Q4 paired gate/up/SwiGLU | A114 precise-exp source retained behind `SGLANG_MLX_NATIVE_Q4_FUSED_SWIGLU` | Eight SIMD groups produce 32 paired rows while sharing each activation load, then materialize the same BF16 gate, up, sigmoid, SiLU, and output boundaries as MLX. A 64-layer trace and boundary negative control pass. Two independent paired windows aggregate to **19.268407916 tok/s**, **+0.206398%** over matched A100+A111 and **0.731592084 tok/s** below the floor. |
